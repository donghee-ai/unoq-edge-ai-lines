...
## 사건
사용자 지침으로 본 작품 3 라인 모두 `history/` + `issues/` 두 폴더를 **`docs/` 하위로 이동**. 가이드 + 트러블슈팅 + 마일스톤이 한 폴더(`docs/`)에 통합되어 외부 진입자 + 멘토 자료 가독성 향상.
...
│   ├── history/
│   ├── issues/
│   └── (mentor/, mentor_style/ — 라인별 멘토 자료)
├── ...
```

장점:
- **외부 진입자 1뷰**: docs/만 보면 가이드 + 마일스톤 + 함정 모두 파악
- **멘토 ZIP 패키지화 단순**: `git archive --format=zip HEAD docs/`로 가이드/이슈/히스토리 한 번에
- **일관성**: 3 라인 동일 구조
