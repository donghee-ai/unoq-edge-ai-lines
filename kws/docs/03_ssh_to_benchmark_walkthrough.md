...
> **아직 측정하지 않았다.** 아래는 "무엇을 보고 무엇을 넘겨야 하는가"만 정의한다.
> 실제 값은 측정 시점에 `benchmarks/`의 JSON과 결과 문서에 남기고, **이 표에는 채워
> 넣지 않는다** — 계획 문서와 결과 문서를 섞으면 나중에 어느 게 실측인지 구분이 안 된다.
> (리포 규칙: 표의 숫자는 우리 실측만, 안 잰 것은 "미측정" — 루트 README 참고)

### 10-1. Latency — 무엇을 보나

`kws_latency_t1.json`(1스레드) · `kws_latency_t4.json`(4스레드)에서 `mean` · `p50` ·
`p95` · `fps_p50`을 읽는다.

**합격 기준: p95 ≤ 50 ms** (두 파일 각각)

### 10-2. 통합 지표 — Pose 단독 대비 증가분

"Pose 단독" 열은 **pose 라인에서 우리가 이미 측정한 실측값**이다. KWS를 함께 돌렸을 때의
값을 이것과 비교한다.

| 지표 | Pose 단독 (실측) | Pose+KWS 합격 기준 |
|---|---|---|
...
### 10-3. False trigger — 오탐 허용치

무발화 상태로 두고 `★ EVENT` 발생 수를 센다.

| 조건 | 시간 | 합격 기준 |
|---|---|---|
| Clean (조용한 실내) | 60 s | detections < 2 |
| 생활 노이즈 | 60 s | detections < 4 |
