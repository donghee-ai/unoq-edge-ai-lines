# 공식 벤치마크 — 호스트 vs UNO Q (멘토 docs 06 형식)

> 본 문서는 멘토 패키지 `06_testing_benchmarking_reliability.md` Section 3의 JSON 형식 권고를 따른 정식 벤치마크 결과를 기록합니다.
> 단일 이미지 100회 반복 측정 (warmup 10 + measure 100), 호스트(Docker)와 UNO Q 디바이스 양쪽에서 수행.
...
- `warmup`, `inference_ms_mean`, `draw_ms_mean`, `stages` (단계별 상세 통계), `last_detections_count`, `thresholds`
...
→ YOLOv8n int8 @ 320×320 / CPU 4 thread / ai_edge_litert + XNNPACK 조합으로 본 작품 운영 최종 확정.
→ 본 프로젝트 의사결정 트리(01)의 MediaPipe 전환 트리거 발동 안 됨.
...
ai_edge_litert는 Google AI Edge의 모던 런타임. tensorflow.lite와 동일 API에 더 최적화.
호스트/디바이스 양측에서 같은 런타임 사용 → 비교 일관성 확보.
...
| 날짜 | 변경 | 사유 |
|---|---|---|
| 2026-06-23 | 초안 작성 | 호스트/디바이스 양측 100회 측정 + 멘토 형식 JSON 저장 완료 |
| 2026-06-23 | 다른 측정 문서 inter-doc 인용 정리 — 본 문서가 공식 e2e 벤치마크의 단일 SoT | 중복 정리 |
| 2026-06-23 | 양식 정리 — 이모지를 텍스트로 대체(통과/수집 등), 굵게 강조 제거 | 본 프로젝트 표준 양식 통일 |
...
|---|---|
| 작성일 | 2026-06-23 |
| 측정 도구 | `src/benchmark_e2e.py` |
| 멘토 docs 참조 | `06_testing_benchmarking_reliability.md` Section 3 (JSON 형식 권고) |
| 적용 범위 | 호스트/디바이스 e2e 공식 벤치마크 (100회) — 본 작품 최종 합격 판정 |
| 갱신 정책 | 모델/런타임/측정 도구 변경 시, 카메라 입력 연결 시 |
