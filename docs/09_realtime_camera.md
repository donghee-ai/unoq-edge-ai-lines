# 카메라 입력 실시간 YOLOv8 (멘토 06 형식)

> 본 문서는 UNO Q 디바이스에 USB UVC 카메라를 연결하여 YOLOv8n int8 TFLite로 실시간 추론한 첫 측정 결과를 기록합니다. 단일 책임 — 카메라 입력 시 e2e 성능 + 운영 안정성 검증.
...
- `capture_ms_mean`, `elapsed_s`, `fps_effective`, `reconnect_count`, `camera` (device + 해상도)
...
첫 프레임의 카메라 init 비용. 안정 후엔 1~2 ms 수준. rolling FPS 이동평균(30프레임)으로 사용자 체감 영향 없음.

필요 시 앱 시작 시 dummy 프레임 N장 캡처으로 warmup 가능.

### 5-2. cv2 drawing cold start 재확인

이전 단계(07)에서 발견한 함정이 카메라 루프에서도 동일하게 나타남 (draw max 56 ms, 첫 프레임). warmup 10회로 충분 제거 가능 (벤치 도구는 그렇게 처리). 실시간 루프에선 첫 프레임만 영향, 평균엔 미미.
...
285초 운영에서 70.8°C 도달 — 합격선(70°C) 1도 초과. QRB2210 안전 영역(~85°C) 내지만 margin 좁아짐.
시간 누적 + 부하 누적 + HTTP 스트리밍 부담 복합 영향. 완화 옵션:
...
| 날짜 | 변경 | 사유 |
|---|---|---|
| 2026-06-23 | 초안 작성 | 카메라 입력 첫 측정 (100 frames, 8.52 FPS) — 4기준 합격 |
| 2026-06-23 | Section 3-3 추가 — 장시간 운영 측정 (2184 f / 285 s / `--serve 8080`) 보강 | FPS 8.29 합격 / temp 70.8°C 임계 1도 초과 발견 — thermal soak test 우선순위 상승 |
| 2026-06-23 | infer_camera.py 보강 — `--serve PORT` HTTP MJPEG 라이브 스트리밍 추가 (DEBUG ONLY) | 호스트 브라우저에서 디바이스 카메라 + 통계 실시간 확인 (로컬 LAN 한정) |
| 2026-06-23 | Section 7-5 추가 — 라이브 디버그 + 자동 저장 권장 명령 + 함정 5종 등록 (uvcvideo reload, pkill self-kill, nested SSH 등) | 실 운영 사고 기록 → 다음 세션 재현성 |
| 2026-06-23 | 양식 정리 — 이모지를 텍스트로 대체(통과/수집/임계 도달 등), 굵게 강조 제거 | 본 프로젝트 표준 양식 통일 |
...
| 작성일 | 2026-06-23 |
| 측정 도구 | `src/infer_camera.py` (멘토 06 권고 준수) |
| 적용 범위 | UNO Q 카메라 입력 실시간 YOLO e2e 측정 (100 frames + 2184 frames 운영) |
| JSON 파일 | `benchmarks/cam_20260623.json`, `cam_serve_20260623_<HHMMSS>.json` (디바이스 → 호스트 회수 필요) |
| 갱신 정책 | 카메라/모델/런타임/해상도 변경 시. soak test 결과 추가 시 |
