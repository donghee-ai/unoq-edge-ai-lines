# Model Selection — YOLOv8n int8 (Primary) and MediaPipe Face (Alternative)

본 문서는 본 프로젝트의 추론 모델 후보 검토 결과와 선택 근거를 정리합니다. 모델 획득 단계에서 YOLO 계열을 1차로 진행하며, 후속에 발생할 수 있는 속도 문제에 대비하여 대안 모델(MediaPipe Face)을 사전 조사하여 함께 기록합니다.

## 0. 핵심 결정

| 항목 | 내용 |
|---|---|
| **1차 채택 모델** | YOLOv8n int8 (Ultralytics 표준 export 경로) |
| **전환 트리거 조건** | UNO Q 실측 FPS가 약 8~10 FPS 미만일 때 |
| **트리거 충족 시 대안** | MediaPipe Face (face_detector + face_landmark) |
| **감정 추론 방식** | L1/L2 규칙 기반 (정규화된 기하 특징, 추론 신경망 X) |
...
- 본 작품 기능 요구사항(사람/얼굴 영역 검출)에 기능적으로 충분.
...
- 모바일 폰 face filter용으로 설계되어 CPU 추론에 최적화.
- 같은 CPU 환경(QRB2210)에서 YOLO보다 빠를 가능성 높음 (단, 실측 필요).
...
- Qualcomm AI Hub의 export 대상 칩 목록에 QRB2210 미지원 확인 (2026-06 기준). AI Hub 우회, MediaPipe 원본에서 직접 획득 필요.
- 공개 벤치마크는 QCS6490 (NPU 보유) 기준으로 503µs / 1988 FPS 표기. QRB2210 (CPU only)에서 50~150× 느려질 것으로 추정.
- 추정 추론 시간: face_detector 5~25 ms + face_landmark 25~80 ms. 합산 10~30 FPS 예상 (실측 필요).
...
- MediaPipe Python 패키지: <https://pypi.org/project/mediapipe/>

### 3-7. 전환 트리거 충족 전까지의 처리
...
### 4-1. 기본 방침 — L1/L2 규칙 기반 (추론 신경망 X)
