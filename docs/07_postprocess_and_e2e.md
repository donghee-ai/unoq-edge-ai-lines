# 후처리 모듈 + End-to-End 측정 (최종 합격 판정)

> 본 문서는 `src/postprocess.py`와 `src/infer_image.py` 작성,
> 작성 중 발견한 두 가지 함정 (정규화 좌표 버그, cv2 cold start),
> 그리고 본 작품의 **최종 end-to-end FPS 판정**을 기록합니다.
...
| 발견 2 — cv2 cold start | cv2 drawing 함수 첫 호출 ~60 ms, warmed 시 ~4 ms |
...
본 프로젝트 코딩 규약(02)에 따라 후처리를 별도 모듈로 분리. 단일 책임 — 디코딩 + NMS + 시각화.
...
**결론**: Ultralytics int8 TFLite export의 출력은 **정규화 [0, 1] 좌표**.
(PyTorch/ONNX export는 픽셀 좌표이지만 TFLite export는 다름.)
...
**모델 export 경로마다 출력 좌표 형식이 다를 수 있음.** 멘토 docs `04_robust_tflite_inference_code.md` Section 5에서 권고한 "출력 shape 먼저 기록 후 디코더 선택"의 정신.
PyTorch checkpoint 또는 ONNX 모델 사용 시 본 decode 함수의 정규화 변환 라인 제거 필요.
...
**draw가 60ms 단독으로 느림.** save는 eMMC 쓰기로 합리적 수준.
...
cv2.rectangle, cv2.putText는 **프로세스 내 첫 호출 시 내부 초기화 비용**:

- 폰트 텍스처 로드
- 그래픽 컨텍스트 준비
- C 라이브러리 dynamic load

`infer_image.py`에서 60ms 나온 이유:

- 그 측정 시점이 cv2 drawing 함수의 진짜 첫 호출
- 진단 스크립트는 cv2.imread, interpreter.invoke 등이 일부 cv2 초기화를 미리 트리거 → 5ms 수준
...
| **실시간 루프 (camera 입력 등)** | 첫 프레임 60ms, **이후 매 프레임 ~4 ms** |
...
| 날짜 | 변경 | 사유 |
|---|---|---|
| 2026-06-23 | 초안 작성 | postprocess + infer_image 작성, bbox 정규화 버그 발견/해결, cv2 cold start 진단, e2e 최종 9.3 FPS 확정 |
| 2026-06-23 | 호스트와 디바이스 종합 비교 섹션 (5-3) 제거 — e2e 측정과 함정 진단에 집중 | 중복 정리 |
...
|---|---|
| 작성일 | 2026-06-23 |
| 관련 파일 | `src/postprocess.py`, `src/infer_image.py` |
| 적용 범위 | 후처리 모듈 구조 + 두 함정(정규화 좌표, cv2 cold start) + e2e 합격 판정 |
| 갱신 정책 | 후처리 모듈/추론 파이프라인 변경 시 |
