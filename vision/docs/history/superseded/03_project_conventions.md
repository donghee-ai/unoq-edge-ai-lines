# Project Coding and Operations Conventions

본 문서는 본 프로젝트의 코드 / 명령어 / 배포 / 문서 작성 규약을 한 곳에 모은 것입니다. 임베디드 AI 프로젝트에서 흔히 권장되는 패턴들을 본 프로젝트의 정식 규약으로 채택한 결과입니다.
...
소유권: `${UNO_Q_USER}` 유저 단독 소유. 권한: 디렉토리 `755`, 파일 `644`, 비밀 파일 `600`.
...
`/opt`와 `/tmp`는 같은 "흩뿌리는 곳"이 아닙니다 — 각각 의미가 다르며 정리 의무도 다릅니다.
...
- 임시 파일 / 실험 출력은 `/tmp/unoq-yolo/` 하위에 둠. 시스템이 자동 정리하므로 영구성을 가정하지 않음. 본인 프로젝트 이름의 서브디렉토리에 모아 두어 작업 중 추적 가능.
...
한 줄 원칙: 어디에 두든 본인 프로젝트 이름이 들어간 서브디렉토리 안. `/opt`는 영구 자산, `/tmp`는 일시 작업.
...
UNO Q의 기본 비밀번호는 첫 접속 시 즉시 변경합니다.

## 4. 모델 및 라이선스 규약

### 4-1. 모델 도입 절차

새 모델을 본 프로젝트에 도입할 때 다음을 `02_model_selection_log.md`에 기록:
...
q-offsets, q-scales, input shape, output shape 등 모델 메타데이터는 절대 코드에 hard-code 하지 않습니다.
...
JSON 포맷은 본 프로젝트 벤치마크 문서(`07_official_benchmark.md`)에서 별도 정의.
...
- 외부 참고자료의 한국어 정리는 `docs/_private_refs/` 하위에 보관 (외부 공개 X).
...
의사결정을 다루는 문서(`02_model_selection_log.md` 등)는 다음 섹션 필수:
...
git에 커밋하지 않고, 다운로드 스크립트(`scripts/download_models.sh`)로 재현. 모델 source URL은 `02_model_selection_log.md`의 관련 링크 섹션에서 관리.
